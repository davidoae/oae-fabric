...
def upgrade():
    ...
    puppet.
       ...
       This will:
        ...
        1.  Stop puppet on the web node. This must be done before the latest
        ...
        2.  Perform a git pull on the puppet node to get the latest
            configuration
        3.  Run the hilary upgrade task on the first PP node in the list. If
        ...
        6.  Run the hilary upgrade task in parallel on the remainder of the
            Hilary nodes, which entails shutting down the Hilary server early
            in the process
        7.  Start puppet on the web node
    """
